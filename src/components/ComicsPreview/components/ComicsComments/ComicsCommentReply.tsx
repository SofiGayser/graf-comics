...
export const ComicsCommentReply: FC<ComicsCommentProps> = ({ comment, replyName, onReply }) => {
  ...
  return (
    <section>
        ...
        <div className={styles['comments__content-body']}>
          ...
          <div>
            ...
            <p className={styles['comments__text']}>
              {replyName && <span className={styles['comments__text-reply']}>{replyName}</span>}
              {replyName && ', '}
              {comment.text}
            </p>
      ...
      {/* Если есть дополнительные ответы, показываем кнопку */}
      {comment.replies && comment.replies.length > 0 && !showMoreReplies && (
        <button className={styles['comments__button-review']} onClick={handleShowMoreReplies}>
          Показать другие ответы ({comment.replies.length})
        </button>
      )}

      {/* Показываем дополнительные ответы если нажали кнопку */}
      {showMoreReplies && comment.replies && comment.replies.length > 0 && (
        <div className={styles['comments__more-replies']}>
